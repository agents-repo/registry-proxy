...
- Edge cache TTL: **300 seconds** (`TAGS_EDGE_TTL_SECONDS`). The worker stores `X-Registry-Proxy-Tags-Cached-At` on cached `/tags` responses and re-fetches upstream when that age exceeds the TTL. Responses include `Cache-Control: public, max-age=300`.
